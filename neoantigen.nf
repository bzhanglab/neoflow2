...
workflow neo_antigen {
  ...
  main:
    ...
    input_ch = hla_type.combine(var_db, by:0)
    ...
    ref_db_ch = Channel.from(sample_exp_mapping)
                    .combine(var_db)
                    .filter{"${it[0]}"=="${it[2]}"}
                    .map{["${it[1]}","${it[3]}"]}
    new_ch_2 = ref_db_ch.combine(split_file.out.var_info_files, by:0)
